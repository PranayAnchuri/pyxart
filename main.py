...
setup_message, secret, creator_key = create_group(clients[1:], names[0], clients[0].get_iden_key_priv())
...
for client, node in zip(clients[1:], leaf_nodes[1:]):
    ...
    recon = process_group_message(group_name, setup_message, client, users)
    assert recon == secret
print("done1")

update_client(clients[1])
updated_tree_secret = update_group_message(group_name, setup_message, clients[1], users)

for client, node in zip(clients[2:], leaf_nodes[2:]):
    path = [_ for _ in create_copath(node)]
    #recon = process_group_message(client, srv, setup_message.initiator, setup_message.setup_key, path)
    #recon = update_group_message(group_name, setup_message, client, users)
    recon = process_group_message(group_name, setup_message, client, users)
