...
class PyxartServicer(pyxart_pb2_grpc.PyxartServicer):
    ...
    def get_users(self, request, context):
        for name, bundle in self.server.clients.items():
            yield pyxart_pb2.ClientRegister(name=name, iden_key_pub=bundle.iden_key_pub, pre_key_pub=bundle.pre_key_pub)
    ...
    def create_group(self, request, context):
        ...
        grp_key, members = self.server.register_group(request.creation_pickle)
    ...
    def get_my_groups(self, request, context):
        for group_key, creation_message in self.server.get_groups(request.name):
            yield pyxart_pb2.GroupInfo(name=pyxart_pb2.GroupName(name=group_key), creation_message=pyxart_pb2.GroupCreation(creation_pickle=creation_message))
